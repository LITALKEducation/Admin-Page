...
export const SERVICE_SURFACES: ServiceSurface[] = [
  'website', 'ask', 'chat_site', 'portal', 'chat_portal', 'checkin', 'booking', 'learning', 'admin',
];
...
export interface ActiveNotice extends ServiceNotice { phase: NoticePhase; }

interface NoticeRow {
  id: number; enabled: number; preset: string; surfaces: string;
  title_th: string; title_en: string; body_th: string; body_en: string;
  announce_from: string | null; starts_at: string | null; ends_at: string | null;
  dismissible: number; updated_at: string | null; updated_by: string | null;
}
...
function phaseFor(notice: ServiceNotice, now: number): NoticePhase | null {
  ...
  if (!Number.isNaN(ends) && now >= ends) return null;
...
export async function listNotices(db: D1Database): Promise<ServiceNotice[]> {
  const { results } = await db.prepare(`SELECT * FROM service_notices ORDER BY COALESCE(starts_at, announce_from, created_at) DESC, id DESC`).all<NoticeRow>();
  return (results ?? []).map(toNotice);
}

export async function activeNotices(db: D1Database, now = Date.now()): Promise<ActiveNotice[]> {
  const { results } = await db.prepare(`SELECT * FROM service_notices WHERE enabled = 1`).all<NoticeRow>();
...
export function blockedMessage(notice: ActiveNotice, lang: string): string {
  ...
  return body || title || (th ? 'ระบบปิดปรับปรุงชั่วคราว กรุณาลองใหม่อีกครั้งภายหลัง' : 'This service is temporarily unavailable. Please try again later.');
}
...
export function sanitizeNotice(raw: unknown): Omit<ServiceNotice, 'id'> {
  ...
  return {
    ...
    surfaces,
    titleTh: text(source.titleTh), titleEn: text(source.titleEn),
    bodyTh: text(source.bodyTh), bodyEn: text(source.bodyEn),
    announceFrom: time(source.announceFrom), startsAt: time(source.startsAt), endsAt: time(source.endsAt),
...
export async function insertNotice(db: D1Database, n: Omit<ServiceNotice, 'id'>, by: string): Promise<number> {
  const result = await db.prepare(`INSERT INTO service_notices
    (enabled, preset, surfaces, title_th, title_en, body_th, body_en, announce_from, starts_at, ends_at, dismissible, updated_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(n.enabled ? 1 : 0, n.preset, JSON.stringify(n.surfaces), n.titleTh, n.titleEn, n.bodyTh, n.bodyEn, n.announceFrom, n.startsAt, n.endsAt, n.dismissible ? 1 : 0, by).run();
  return Number(result.meta?.last_row_id ?? 0);
}

export async function updateNotice(db: D1Database, id: number, n: Omit<ServiceNotice, 'id'>, by: string): Promise<void> {
  await db.prepare(`UPDATE service_notices SET enabled = ?, preset = ?, surfaces = ?, title_th = ?, title_en = ?, body_th = ?, body_en = ?, announce_from = ?, starts_at = ?, ends_at = ?, dismissible = ?, updated_at = CURRENT_TIMESTAMP, updated_by = ? WHERE id = ?`)
    .bind(n.enabled ? 1 : 0, n.preset, JSON.stringify(n.surfaces), n.titleTh, n.titleEn, n.bodyTh, n.bodyEn, n.announceFrom, n.startsAt, n.endsAt, n.dismissible ? 1 : 0, by, id).run();
}
...
export async function disableBlockingNotices(db: D1Database, by: string): Promise<number> {
  ...
  await db.batch(blocking.map((n) => db.prepare(`UPDATE service_notices SET enabled = 0, updated_at = CURRENT_TIMESTAMP, updated_by = ? WHERE id = ?`).bind(by, n.id)));
...
export async function rotateBypassToken(db: D1Database): Promise<string> {
  ...
  await db.prepare(`UPDATE service_settings SET bypass_token = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1`).bind(token).run();
  return token;
}
